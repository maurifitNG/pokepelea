...
class Move():
    ...
    def __init__(self, url):
        ...
        #llamado de los movimientos Api endpoint
...
class Pokemon(pygame.sprite.Sprite):
    ...
    def __init__(self, name, level, x, y):
        ...
        # llamar al pokemon API endpoint
        req = requests.get(f'{base_url}/pokemon/{name.lower()}')
        self.json = req.json() 
        # set nombre y level del pokemon
        self.name = name
        self.level = level  
        # setear la posicion del sprite en la pantalla
        self.x = x
        self.y = y
        # numero de pociones
        self.num_potions = 3
        
        # obtener status del pokemon con la API
        ...
        # set sprite's width
        self.size = 150
        # set el sprite con cara de frente
    ...
    def perform_attack(self, other, move):
        ...
        # bono de daño por mismo tipo (STAB)
    ...
    def use_potion(self):  
        # revisar si quedan pociones
        if self.num_potions > 0:
            
            #  añade 30 de hp(pero no supera el hp max)
            self.current_hp += 30
            if self.current_hp > self.max_hp:
                self.current_hp = self.max_hp   
            # disminuye la cantidad de pociones tras el uso
            self.num_potions -= 1
    ...
    def set_moves(self):
        ...
        self.moves = []
        # obtiene todos los movimientos a travez de la api
        for i in range(len(self.json['moves'])):    
            # obtiene los movimientos de distintas versiones del juego
            versions = self.json['moves'][i]['version_group_details']
            for j in range(len(versions)):
                
                version = versions[j]  
                # solo obtiene moves de la version red-blue
                if version['version_group']['name'] != 'red-blue':
                    continue     
                # solo movimientos obtenidos por lvl up (excluye TM moves) 
                learn_method = version['move_learn_method']['name']
                if learn_method != 'level-up':
                    continue
                # añade el movimiento si el lvl es suficientemente alto
        ...
        # selecciona 4 movimientos al alzar
        if len(self.moves) > 4:
...
def display_message(message):
    # dibuja caja blanco con bordes negros
...
# creacion de los pokemones iniciales
level = 30
...
# eleccion  de pokemon del jugador y del rival
player_pokemon = None
rival_pokemon = None
